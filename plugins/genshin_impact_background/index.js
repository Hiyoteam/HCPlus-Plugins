(async function () {
  var people_list = [
    ...
    .map((e) => e.list.map((u) => ({ ...u, city: e.city })))
    .flat();

  var host = `https://plugins.hach.chat/plugins/genshin_impact_background/`;

  var cdn_list = [
    `https://camo.hach.chat/?proxyUrl=https://raw.githubusercontent.com`,
  ...
  var img_valid = (j, e) =>
    ...
    new Promise(
      (p, o) =>
        !!(e.onload = () => p((e.__ok = true))) &&
        !!(e.onerror = () => p((e.__ok = false))) &&
        !!setTimeout(() => p(e.__ok ?? false), 20 * 1000) &&
        undefined
    );
  var cdn_test_png = `redble/cdn_genshin_impact/main/asset/liyue/hutao.png`;

  var cdn_get = async () => {
    for (let url of cdn_list) {
      let res = await img_valid(`${url}/${cdn_test_png}`);
      if (res) return url;
    }
  };
  var cdn_host_get = async () =>
    localStorage["genshin_impact_background_init"] == "true"
      ? localStorage["genshin_impact_cdn"] == "auto"
        ? await cdn_get()
        : localStorage["genshin_impact_cdn"]
      : (localStorage["genshin_impact_cdn"] = await cdn_get());
  var cdn_host = await cdn_host_get();
  var update_cdn_host = async () =>
    (localStorage["genshin_impact_cdn"] = cdn_host = await cdn_get());
  if (localStorage["genshin_impact_background_init"] != "true")
    localStorage["genshin_impact_background_init"] = "true";
  var img_host = () => `${cdn_host}/redble/cdn_genshin_impact/main/asset/`;
  var selected_people = localStorage["_gs_cache_name"] || "hutao";
  var opacity = localStorage["_gs_bg_opacity"] || "30";
  var push_info = (e) => pushMessage({ nick: "*", text: e }),
    push_error = (e) => pushMessage({ nick: "!", text: e });
  var setPeople = (e) => {
  ...
  var find_people = (e) => {
    ...
    return {
      ...people,
      path: {
        pc: img_host() + people.title + ".png",
        phone: img_host() + people.title + ".2.png",
      },
      find: true,
    };
  };
  ...
  async function useCache(name) {
    ...
    if (
      !conf.cache ||
      !conf.cache_name ||
      !conf.cache_pc_base64 ||
      !conf.cache_phone ||
      conf.cache_name != name
    ) {
      localStorage["_gs_cache"] = "true";
      localStorage["_gs_cache_name"] = name;
      try {
        conf.cache_pc_base64 = localStorage["_gs_cache_pc"] = await getImage(
          name,
          "0"
        );
        conf.cache_phone = localStorage["_gs_cache_phone"] = await getImage(
          name,
          "1"
        );
      } catch {
  ...
  function init() {
    ...
    injectSelect(
      "Background Image",
      people_list.map((e) => [e.name, e.title]),
      (e) => {
    ...
    injectSelect(
      "Opacity",
      [...Array(11)].map((e, i) => [(i * 10).toString() + "%", i * 10]),
      (e) => {
    ...
    injectSelect(
      ...
      (e) => {
        container_obj.style.zIndex =
          (localStorage["_gs_bg_zindex"] = e.target.value) == "0"
            ? "-9999999"
            : "9999999";
      },
      (e) => {
        container_obj.style.zIndex =
          (e.value = localStorage["_gs_bg_zindex"] || "1") == "0"
            ? "-9999999"
            : "9999999";
        return e;
      }
  ...
  injectSelect(
    ...
    [
      ["Auto", "auto"],
      ["Camo", cdn_list[0]],
      ["Github Proxy", cdn_list[1]],
      ["Raw", cdn_list[2]],
    ],
    async (e) => {
      cdn_host =
        (localStorage["genshin_impact_cdn"] = e.target.value) == "auto"
          ? await cdn_get()
          : e.target.value;
    },
    (e) => {
      e.value = localStorage["genshin_impact_cdn"];
      temp_cdn_list = e;
      return e;
    }
  );
  addPluginButton("Refresh CDN route", async () => {
    let t = (temp_cdn_list.value = await update_cdn_host());
    push_info(`Selected Best CDN Route: ${t}`);
  });
  addPluginButton("Reload image", () => {
    push_info("Reloading...");
